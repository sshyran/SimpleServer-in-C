...
namespace TestApp
{
    public class TestHandler2 : IHandler
    {
        ...
        {
            if (context.Request.FormattedUrl != "/") return;
            if (context.Request.Method == "GET")
            {
                var sw = new StreamWriter(context.Response.OutputStream);
                sw.WriteLine(
                    "<h1>Hello, Local World!</h1><form action=\"/\" method=\"POST\"><a>What's your name?</a><input type=\"text\" name=\"name\" value=\"Dylan\" /><input type=\"submit\" /></form>");
                sw.Flush();
                sw.Close();
