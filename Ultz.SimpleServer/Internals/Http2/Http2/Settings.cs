...
namespace Ultz.SimpleServer.Internals.Http2.Http2
{
    ...
    public struct Settings
    {
        ...
        ///     Contains the default settings
        /// </summary>
        public static readonly Settings DefaultSimpleServer = new Settings
        {
            HeaderTableSize = 4096,
            EnablePush = true,
        ...
        /// </summary>
        public static readonly Settings Min = new Settings
        {
            HeaderTableSize = 0,
            EnablePush = false,
        ...
        /// </summary>
        public static readonly Settings Max = new Settings
        {
        ...
        public Http2Error? UpdateFromData(ArraySegment<byte> data)
        {
            ...
            for (var o = data.Offset; o < data.Offset + data.Count; o = o + 6)
            {
                ...
                // Update the value
                var err = UpdateFieldById(id, value);
        ...
        public Http2Error? UpdateFieldById(ushort id, uint value)
        {
            switch ((SettingId) id)
            {
                ...
                case SettingId.HeaderTableSize:
                    if (value >= Min.HeaderTableSize &&
                        value <= Max.HeaderTableSize)
                    {
                ...
                case SettingId.InitialWindowSize:
                    if (value >= Min.InitialWindowSize &&
                        value <= Max.InitialWindowSize)
                    {
                ...
                case SettingId.MaxConcurrentStreams:
                    if (value >= Min.MaxConcurrentStreams &&
                        value <= Max.MaxConcurrentStreams)
                    {
                ...
                case SettingId.MaxFrameSize:
                    if (value >= Min.MaxFrameSize &&
                        value <= Max.MaxFrameSize)
                    {
                        MaxFrameSize = value;
                        return null;
                    }
                ...
                case SettingId.MaxHeaderListSize:
                    if (value >= Min.MaxHeaderListSize &&
                        value <= Max.MaxHeaderListSize)
                    {
            ...
            if ((SettingId) id == SettingId.InitialWindowSize) code = ErrorCode.FlowControlError;
