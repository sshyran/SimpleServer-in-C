...
namespace Ultz.SimpleServer.Internals.Http2.Http2
{
    ...
    internal class StreamImpl : IStream
    {
        ...
        private readonly int totalReceiveWindow;
        ...
        private ReceiveQueueItem receiveQueueHead;
        private int receiveWindow;
        private StreamState state;
        private long totalInData;
            ...
            int receiveWindow)
        {
            ...
            totalReceiveWindow = receiveWindow;
        }
            ...
            IEnumerable<HeaderField> headers, bool endOfStream)
        {
            ...
            if (hvr != HeaderValidationResult.Ok) throw new Exception(hvr.ToString());
        ...
        {
            var hvr = HeaderValidator.ValidateTrailingHeaders(headers);
            // TODO: For push promises other validates might need to be used
            if (hvr != HeaderValidationResult.Ok) throw new Exception(hvr.ToString());
            ...
            try
            {
                ...
                lock (stateMutex)
                {
                    if (!dataSent) throw new Exception("Attempted to write trailers without data");
            ...
            {
                writeMutex.Release();
                if (removeStream) connection.UnregisterStream(this);
            }
        }
        ...
        public async ValueTask<StreamReadResult> ReadAsync(ArraySegment<byte> buffer)
        {
            while (true)
            {
                ...
                lock (stateMutex)
                {
                    if (state == StreamState.Reset) throw new StreamResetException();
                    ...
                    if (receiveQueueHead != null)
                    {
                        ...
                        if (receiveQueueHead == null && !streamClosedFromRemote) readDataPossible.Reset();
                    }
                ...
                if (hasResult)
                {
                    if (windowUpdateAmount > 0) await SendWindowUpdate(windowUpdateAmount);
                    return result;
                }
            }
        }
        ...
        public async Task WriteAsync(ArraySegment<byte> buffer, bool endOfStream = false)
        {
            ...
            try
            {
                lock (stateMutex)
                {
                    ...
                    if (!headersSent) throw new Exception("Attempted to write data before headers");
        ...
        {
            return WriteAsync(Constants.EmptyByteArray, true);
        }
            ...
            IEnumerable<HeaderField> headers, bool endOfStream)
        {
            ...
            if (res != ConnectionWriter.WriteResult.Success) throw new Exception("Can not write to stream");
        }
            ...
            IEnumerable<HeaderField> headers, bool endOfStream)
        {
            ...
            try
            {
                ...
                lock (stateMutex)
                {
                    ...
                    if (dataSent) throw new Exception("Attempted to write headers after data");
            ...
            ErrorCode errorCode, bool fromRemote)
        {
            ...
            lock (stateMutex)
            {
                if (state == StreamState.Reset || state == StreamState.Closed) return writeResetTask;
        ...
        private async ValueTask<object> SendWindowUpdate(int amount)
        {
            ...
            try
            {
                await connection.writer.WriteWindowUpdate(fh, updateData);
            }
            catch (Exception)
            {
            ...
            CompleteHeadersFrameData headers)
        {
            ...
            if (wakeupHeaderWaiter) readHeadersPossible.Set();
            if (wakeupDataWaiter) readDataPossible.Set();
            if (wakeupTrailerWaiter) readTrailersPossible.Set();
